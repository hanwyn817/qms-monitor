...
- 自动统计：
  ...
  - 超期起数与占比
  - 超期清单
  - 超期按分管 QA、分管 QA 中层、责任部门降序统计
- 对分管 QA / 分管 QA 中层按超期起数前 20% 分位阈值（含并列）人员做超期内容 LLM 概括（“主要为xxx”短语）
...
默认读取项目根目录下 `config.xlsx`。最新版本已支持**智能表头名称映射**，不再强制要求特定列序号顺序。由于系统自动通过带有特定词汇的表头进行定位，只需确保表中第一行存在以下核心参数概念即可（推荐结构如下）：
...
- 分管 QA（或包含“分管QA”）
- 分管 QA 中层（或包含“分管QA中层”）
- 未完成状态（或包含“未完成”，必填；用于状态判定）
- 数据起始行（可选；默认 2，或包含“起始行”）

实现规则：

- 若 `J` 列为字母列标（如 `J`、`AA`），按该列提取的计划日期判定超期（仅支持字母列标）。
- 若 `J` 列为数字，视为完成时限（天），计划日期 = 发起日期 + 天数。
- 若 `J` 列为空，默认计划日期 = 发起日期 + 1 个月（兼容旧逻辑）。
...
- 项目内置了本地 Excel 读取模块 `qms_monitor/excel_reader.py`，不再依赖外部 Excel 读取包。新版增加了深度防卡死、COM 对象强制销毁功能及 `Interactive=False`，有效避免僵尸进程。
- `csv` 模式会按 `config.xlsx` 的 `row_no` 关联到 `manifest.json` 中的导出文件。
- 本仓库不在设计阶段主动读取真实质量台账文件；实际运行时按配置执行。
- **并发加速**：LLM 调用已全面支持多线程并行分析不同模块的内容概括，从而可成倍节省大批量模块数据生成报告时的等待成本。
